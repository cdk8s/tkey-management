...
public enum ExceptionDescriptionEnum {
	...
	ERROR_UPDATING_DATABASE("Error updating database", "操作数据库失败，请联系管理员进行处理"),
	...
	NUMBERFORMAT_ERROR("For input string:", "输入格式有误"),
	DATA_SIZE_TOO_LONG("java.sql.SQLException: Data too long for column", "存在某个字段过长"),
	//比如有字段是不能为 null 的，但是插入的 sql 语句中是 null
	...
	SERVICE_NOT_AVAILABLE("com.netflix.client.ClientException: Load balancer does not have available management for client", "存在某些服务不可用"),
	SERVICE_TIMEOUT("java.util.concurrent.TimeoutException", "10000服务连接超时"),
	MYSQL_CONNCTION_TIMEOUT("Could not open JDBC Connection for transaction; nested exception is com.mysql.jdbc", "JDBC 服务连接超时"),
	REDIS_CONNCTION_TIMEOUT(" redis.clients.jedis.exceptions.JedisConnectionException", "Redis 服务连接超时"),
	RABBITMQ_CONNCTION_TIMEOUT("org.springframework.amqp.AmqpTimeoutException: java.util.concurrent.TimeoutException", "MQ 服务连接超时"),
	FEIGN_FAIL_AND_NO_FAILBACK("failed and no fallback available", "Feign 连接失败且无回退方法"),
	FEIGNN_FAIL_AND_FAILBACK_AVAILABLE("failed and fallback failed", "Feign 连接失败且回退失败");
